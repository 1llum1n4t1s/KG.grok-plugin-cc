...
- 読み取り専用の Grok レスキュー・レビューで公開 URL の `WebFetch` を使えるようにし、外部資料の取得時に権限拒否で中断する問題を修正しました。URL 以外の追加引数や認証情報を含む URL は拒否します。

## [1.0.19] — 2026-09-25

- Grok の読み取り専用レビュー・監査で、リポジトリ内の単一パスに対する PowerShell の `Test-Path -LiteralPath` を、絶対パスでも許可するようにしました。リポジトリ外や外部へつながるリンク経由のパスは引き続き拒否します。
- 絶対パスなどを拒否した場合の理由を、実際の判定に合う文言へ修正しました。

## [1.0.18] — 2026-09-24

- Windows で Grok ジョブをキャンセルするとき、終了済みの子プロセスに対する `taskkill` の失敗を再確認し、停止済みなら保留状態を残さないようにしました。セッション終了時も同じ方法で処理します。
- 読み取り専用レビュー・監査で、外部コマンド実行や書き込みにつながるシェル引数と ACP のファイル書き込み要求を拒否するようにしました。権限拒否がある結果は未完了として表示します。
- Windows で Git Bash の `SHELL` 設定があっても Grok の確認コマンドを起動できるようにし、ACP 接続失敗時に子プロセスが残らないようにしました。
- リポジトリ全体の監査など、対象範囲を明示した実行で不要な Git の作業ツリー走査を省きました。

## [1.0.17] — 2026-09-24

- Windows で一時的にプロセス情報を取得できない場合も、Grok のレスキューなどのジョブを起動できるようにしました。

## [1.0.16] — 2026-09-23

- Grok 4.7 を既定モデルと `latest` の参照先に設定しました。明示的なモデル指定と環境変数による上書きは引き続き利用できます。
- 読み取り専用レビューで、環境変数の前置きや外部実行・書き込みにつながる検索コマンドを拒否するようにしました。
- ジョブのキャンセルとセッション終了時に実行中プロセスを正しく照合し、対象の Grok タスクを停止できるようにしました。

## [1.0.15] — 2026-09-12

- 権限拒否や調査未完了があるレビュー・監査を、承認ではなく `incomplete` として扱うよう修正しました。JSON 再出力後も拒否理由を保持します。
- 終了時レビューゲートが権限拒否後の `ALLOW` を成功扱いせず、保存結果でも未完了と理由を表示するよう修正しました。
- 読み取り専用レビュー・監査では、利用できないサブエージェント委任を避け、一つの Grok セッションで直接調査するよう案内しました。

## [1.0.14] — 2026-09-12

- Claude Code 専用の内部 helper を Codex の公開 Skill から分離し、Codex がそれらを公開 Skill として読み込まないようにしました。
- `/grok:rescue` への明示的な実装依頼が、レビュー専用ワークフローの変更禁止ルールで誤って止められないようにしました。

## [1.0.13] — 2026-09-11

- レビュー・監査のJSON再出力に失敗した場合、最初の応答ではなく最新の応答とエラーを保存するよう修正しました。
- 保存結果にGrokの終了理由を追加し、処理の完了・中断とJSON解析失敗を区別できるようにしました。
