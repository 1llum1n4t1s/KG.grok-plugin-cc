...
- If no ID is supplied, let the companion cancel the only active job in the current session. When
  multiple jobs are active, require a job ID and direct the user to
  `grok:source-command-status` to disambiguate.
