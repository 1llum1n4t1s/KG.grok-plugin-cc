...
argument-hint: "[--resume|--fresh] [--model <model|fast|reasoning|multi|build|latest>] [--effort <low|medium|high>] [what Grok should investigate, solve, or continue]"
...
- Always run the `grok:grok-rescue` subagent in the foreground. Do not use a background task or a detached process.
- If the request includes `--background`, stop and say that background execution is no longer supported.
- Treat a legacy `--wait` as a no-op. Do not forward it to `task` or include it in the natural-language task text.
