...
- review、adversarial-review、audit、X 検索は読み取り専用で実行する。レビューの権限処理は、コマンド名に加えて引用・引数・パスも検査し、外部コマンド実行や書き込みにつながるシェル要求を拒否する。ACP のファイル書き込み要求もセッションの書き込み権限で判定し、読み取り専用セッションでは拒否と未完了の記録を残す。書き込み可能な委任タスクとは権限経路を分ける。
