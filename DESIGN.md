...
- セッション終了やキャンセルは追跡された所有関係に従う。ジョブを実行中として登録する前にプロセス開始識別子の取得を短時間再試行し、取得できなければ実行を開始せず実行中へ遷移させない。停止時には PID・開始識別子・companion のコマンドラインを照合する。一致しない PID は停止せず、再利用された PID による誤停止を防ぐ。両ホストの保存先が独立する構成では、一方の終了が他方のジョブやブローカー状態を消さない。
