...
- セッション終了やキャンセルは追跡された所有関係に従う。実行中ジョブはプロセス開始識別子を取得できたときだけ登録し、停止時には PID・開始識別子・companion のコマンドラインを照合する。一致しない PID は停止せず、再利用された PID による誤停止を防ぐ。両ホストの保存先が独立する構成では、一方の終了が他方のジョブやブローカー状態を消さない。
